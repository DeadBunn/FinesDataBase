...
class Fine{
    ...
    protected void pay(){
        paid=true;
        getPaymentDay = dateFormat.format(new Date());
    }
    protected void send(){subpoena=true;}
    public String toString (){
        return "ID: " + id + "; Нарушитель: " + intruderFio + "; Сотрудник ГАИ: " + policeFio + "; Номер машины: " + carNumber + "; Сумма штрафа: " + sum + "; Срок оплаты: " + dateFormat.format(deadline) + "; Время составления протокола: " + timeFormat.format(time) + (((subpoena) ? "; Есть повестка в суд" : "; Нет повестки в суд") + ((paid)? "; Штраф оплачен, дата оплаты " + getPaymentDay : "; Штраф не оплачен"));
    }
